...
import { deployCamel } from './deploy.mjs';
...
let forkUrl = cliArgs['fork-url'] || process.env.MAINNET_RPC_URL || process.env.FORK_RPC_URL || 'https://eth.llamarpc.com';
...
const stage = cliArgs.stage || 'pre-launch';
const validStages = ['pre-launch', 'mints', 'trading', 'full'];
if (!validStages.includes(stage)) {
  console.error(`[chain] Invalid stage '${stage}'. Must be one of: ${validStages.join(', ')}`);
  process.exit(1);
}
if (stage !== 'pre-launch') {
  console.log(`[chain] Stage: ${stage}`);
}

// Force chain ID via RPC — anvil ignores --chain-id on forks in some versions
if (chainId !== '31337') {
  try {
    await fetch(rpcUrl, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', method: 'anvil_setChainId', params: [parseInt(chainId)], id: 1 })
    });
    console.log(`[chain] Chain ID set to ${chainId}`);
  } catch (e) {
    console.warn(`[chain] Failed to set chain ID: ${e.message}`);
  }
}

await deployCamel({ silent: cliArgs.silent, rpcUrl, stage });
...
function buildAnvilArgs({ fork }) {
  ...
  if (fork) {
    ...
    // Resilience: prevent deadlock when the fork RPC connection drops.
    // Default timeout is 45s which can hang on closed sockets; 10s fails fast.
    // Retries with backoff let Anvil reconnect on a fresh socket.
    args.push('--timeout', '10000');
    args.push('--retries', '10');
    args.push('--fork-retry-backoff', '500');
  }
  return args;
}
